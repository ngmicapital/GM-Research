...
// the content object. Validation failures (missing section, too few cards, bad
// gm-meta) must throw, never silently degrade — that is the guard against the
// Codex-style fidelity collapse.
  ...
  return {
    ...
    sections: [
      { number: 'Section 01', title: 'The Story', blocks: [
        ...
        { type: 'p', html: 'A second paragraph of narrative.' },
      ]},
      { number: 'Section 02', title: 'The Mechanism', blocks: [
        { type: 'p', html: 'The chemistry of how the scent is released.' },
        { type: 'data_callout', label: 'The molecule', html: 'Geosmin is detectable by humans at five parts per trillion.' },
      ]},
      { number: 'Section 03', title: 'The Connections', blocks: [
        { type: 'p', html: '<strong>Markets.</strong> Signal hidden in noise, like a faint smell after rain.' },
      ]},
    ],
    cards: [
...
test('renders all sections, stats, cards and sources into the template', () => {
  ...
  // exactly 4 grid cards
  assert.strictEqual((html.match(/<div class="card">/g) || []).length, 4);
  // sources rendered as list items with real hrefs
